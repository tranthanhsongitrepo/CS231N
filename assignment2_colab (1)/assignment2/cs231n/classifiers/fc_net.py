...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += 1 / 2 * self.reg * np.sum(self.params['W2'] ** 2) + 1 / 2 * self.reg * np.sum(self.params['W1'] ** 2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers):
          scores, cache_affine = affine_forward(scores, self.params['W' + str(layer)], self.params['b' + str(layer)])
          cache_batchnorm = None
          cache_dropout = None
          if self.normalization == 'batchnorm' and layer < self.num_layers:
              scores, cache_batchnorm = batchnorm_forward(scores, self.params['gamma' + str(layer)], self.params['beta' + str(layer)], self.bn_params[layer - 1])
        ...
        # print(self.num_layers)
        # print(caches[self.num_layers - 1][0].shape, dX.shape)
        ...
        # print(caches[0][1]['gamma'])
        for layer in range(self.num_layers - 1, 0, -1):
          # print(layer)
          dX = relu_backward(dX, caches[layer - 1][2])
          if self.use_dropout:
            dX = dropout_backward(dX, caches[layer - 1][3])
          # print(self.num_layers)
          if self.normalization == 'batchnorm':
            dX, dgamma, dbeta = batchnorm_backward(dX, caches[layer - 1][1])
          dX, dW, db = affine_backward(dX, caches[layer - 1][0])
